        ...
        for (y = 2; y < h; ++y) {
            if (data[y * 3 * w + x * 3 + 1] < 255) {
                if (data[y * 3 * w + x * 3 + 0] == 255) { // red
                    int len = 0;
                    while (y + len < h && data[(y + len) * 3 * w + x * 3 + 0] == 255 &&
                           data[(y + len) * 3 * w + x * 3 + 1] == 0) {
                ...
                if (data[y * 3 * w + x * 3 + 2] == 255) { // blue
                    int len = 0;
                    while (x + len < end_x && data[y * 3 * w + (x + len) * 3 + 2] == 255 &&
                           data[y * 3 * w + (x + len) * 3 + 1] == 0) {
...
int main(int argc, char **argv) {
    ...
    printf("%d\n", sizeof(segments[0]) * stb_arr_len(segments) + sizeof(segments[0]) * stb_arr_len(vsegments) +
                   sizeof(charinfo[32]) * 95);
