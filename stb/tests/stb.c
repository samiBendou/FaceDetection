...
int test_compression(char *buffer, int length) {
    ...
    if (memcmp(buffer, storage2, length) != 0) {
        free(storage2);
        return -1;
    }
    free(storage2);
...
stb_bst(Btest, btest_, BT2, bt2, v,
int, a - b)
...
stb_define_sort(sort_int,
int, *a < *b)

stb_rand_define(prime_rand,
1)
...
Parser *parser_create(short **productions, int num_prod, int start_nt, int end_term) {
    ...
    free(mini_rule);
    stb_pointer_array_free(g.first, num_symbols);
    free(g.first);
    stb_pointer_array_free(g.follow, num_symbols);
    free(g.follow);
    stb_arr_free(g.prod);
    for (i = 0; i < num_symbols; ++i)
