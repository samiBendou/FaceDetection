...
void setup_uniforms(float pos[3]) {
    ...
    for (i = 0; i < STBVOX_UNIFORM_count; ++i) {
        ...
        if (ui) {
            ...
            switch (ui->type) {
                case STBVOX_UNIFORM_TYPE_sampler:
                    stbglUniform1iv(uniform_loc[i], ui->array_length, data);
                    break;
                case STBVOX_UNIFORM_TYPE_vec2:
                    stbglUniform2fv(uniform_loc[i], ui->array_length, data);
                    break;
                case STBVOX_UNIFORM_TYPE_vec3:
                    stbglUniform3fv(uniform_loc[i], ui->array_length, data);
                    break;
                case STBVOX_UNIFORM_TYPE_vec4:
                    stbglUniform4fv(uniform_loc[i], ui->array_length, data);
                    break;
            }
        }
    }
...
void make_texture_buffer_for_uniform(int uniform, int slot) {
    ...
    glBufferDataARB(GL_ARRAY_BUFFER_ARB, ui->array_length * ui->bytes_per_element, ui->default_value,
                    GL_STATIC_DRAW_ARB);
    glBindBufferARB(GL_ARRAY_BUFFER_ARB, 0);
...
void render_init(void) {
    ...
    for (i = 0; i < 256; ++i) {
        ...
        glTexSubImage3DEXT(GL_TEXTURE_2D_ARRAY_EXT, 0, 0, 0, i, TEX_SIZE, TEX_SIZE, 1, GL_RGBA, GL_UNSIGNED_BYTE,
                           texture[0]);
    }
...
void render_caves(float campos[3]) {
    ...
    {
        float lighting[2][3] = {{campos[0], campos[1], campos[2]},
                                {0.75,      0.75,      0.65f}};
        float bright = 8;
        lighting[1][0] *= bright;
        lighting[1][1] *= bright;
